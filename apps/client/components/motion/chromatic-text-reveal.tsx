...
export type ChromaticTextRevealProps = {
  ...
  words: string[]
  /** Breaks the prefix onto its own line above the words. */
  stacked?: boolean
...
export function ChromaticTextReveal({
  ...
  return (
    <span
      ...
      className={cn(
        "inline-flex",
        // Not `stretch`: the word slot positions its sweeping copy against its
        // own left edge, so it has to stay shrink-to-fit to line up with the
        // invisible spans sizing it. Override with `items-start` to left-align.
        stacked ? "flex-col items-center" : "items-baseline",
        className
      )}
    >
      <span className="whitespace-nowrap">
        {prefix}
        {/* Separates the two on one line. On separate lines it would be a
            trailing space that pulls the prefix off-center. */}
        {hasWords && !stacked ? "\u00A0" : null}
      </span>
      {hasWords ? (
