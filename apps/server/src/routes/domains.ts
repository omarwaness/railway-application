...
app.get("/", sValidator("query", scopeSchema), async (c) => {
...
app.get("/available", sValidator("query", z.object({ domain })), async (c) => {
    const client = createRailwayClient(c.get("railwayToken"));
...
app.post("/service", sValidator("json", createServiceDomainSchema), async (c) => {
    const client = createRailwayClient(c.get("railwayToken"));
...
app.delete("/service/:id", sValidator("param", idParamSchema), async (c) => {
    const client = createRailwayClient(c.get("railwayToken"));
    const { id } = c.req.valid("param");
...
app.post("/custom", sValidator("json", createCustomDomainSchema), async (c) => {
    const client = createRailwayClient(c.get("railwayToken"));
    const input = c.req.valid("json");
...
app.delete("/custom/:id", sValidator("param", idParamSchema), async (c) => {
    const client = createRailwayClient(c.get("railwayToken"));
    const { id } = c.req.valid("param");
