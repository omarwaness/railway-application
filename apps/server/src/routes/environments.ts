...
const routes = app
    ...
    .get("/", sValidator("query", listEnvironmentsSchema), async (c) => {
    ...
    // in the dashboard.
    .get("/:id/staged-changes", sValidator("param", idParamSchema), async (c) => {
        const client = createRailwayClient(c.get("railwayToken"));
        const { id } = c.req.valid("param");
    ...
    .post("/", sValidator("json", createEnvironmentSchema), async (c) => {
        const client = createRailwayClient(c.get("railwayToken"));
